...
bool StartGame()
{
    ...
    float V=0.25; ////For Background and Enemies
    float dV=0.00001; ////For Background and Enemies
    ...
    while (window.isOpen())
    {
        ...
        ////Crawling
        if(OnFloor==true&&Life==true&&(sf::Keyboard::isKeyPressed(sf::Keyboard::Down))){
            Crawling=true;
        ...
        ////Jump
        if(sf::Keyboard::isKeyPressed(sf::Keyboard::Space)&&OnFloor==true&&Life==true) {
            OnFloor = false;
            Jump.play();
            dy=-0.5;
            Dino.setTexture(texture1);
